...
int main(int argc, char *argv[]) {
  ...
  MPI_Bcast(&h, 1, MPI_INT, MAITRE, MPI_COMM_WORLD);
    ...
    //on rajoute des lignes aux extrémités pour le calcul de la convolution
    if ((rank == 0) || (rank == p - 1)) {
      h_loc = h/p + 1;
  ...
  MPI_Scatter(r.data,w * h/p,MPI_UNSIGNED_CHAR,ima_loc + w_scatter,w * h/p,MPI_UNSIGNED_CHAR,MAITRE, MPI_COMM_WORLD);
  ...
  MPI_Gather(ima_loc + w_scatter,w * h/p,MPI_UNSIGNED_CHAR,r.data,w * h/p,MPI_UNSIGNED_CHAR,MAITRE,MPI_COMM_WORLD);
