 ...
 * \end{array}\right.
 * \f]
 * le p d'itérations que la suite met pour diverger est le
 * p \f$ n \f$ pour lequel \f$ |z_n| > 2 \f$.
 * Ce p est ramené à une valeur entre 0 et 255 correspond ainsi a
...
int main(int argc, char *argv[]) {
    ...
    MPI_Init(&argc, &argv);
    MPI_Comm_size(MPI_COMM_WORLD, &p);//p de processus
    ...
    ima=malloc(((w*h)/p)*sizeof(unsigned char));
    /* MAITRE */
    if (rank == 0)
      {
        while (indice_bloc < (h/nbLignes))
      {
        ...
        for (j=1; j<p; j++)
      {
        //on envoie à l'esclave le numéro de bloc à traiter
        MPI_Recv(&i, 1, MPI_INT, MPI_ANY_SOURCE, REQ, MPI_COMM_WORLD, &status);
        MPI_Recv(&pima[i*nbLignes*w], w*nbLignes, MPI_CHAR, status.MPI_SOURCE, DATA, MPI_COMM_WORLD, &status);
    ...
    if (rank != 0)
      {
        ...
        //tant qu'il y du travail pour l'esclave
        while (tag != END)
        {
        ...
        //envoie des résultats au maître, ainsi que du numéro de bloc
    ...
    sauver_rasterfile( "mandel.ras", w, h, pima);
